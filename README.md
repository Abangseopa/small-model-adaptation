...
│   └── train.py
├── inference/                  # Stage 5 eval + Stage 7 user-facing inference
│   ├── evaluate_adapted.py     #   Stage 5 — adapted model vs. Step 3 held-out set
│   ├── adapted_model.py        #   Stage 7 — reusable AdaptedModel (load once, .ask(...))
...
in Stage 7 once `run_inference.py` became the project's user-facing local
inference entry point. The frozen Step 5 results were not regenerated by
this rename.
...
`inference/evaluate_adapted.py` (named `run_inference.py` at the time
this evaluation was run — see the Stage 7 note above) attaches the Stage
4 LoRA adapter to a freshly loaded copy of the base model and runs it —
for the first time —
...
Run `python3 inference/evaluate_adapted.py` (needs Stage 4's adapter to
...
## Usable adapted-model inference (Stage 7)

`inference/run_inference.py` is the project's user-facing local inference
tool — not another evaluation. It never touches, reruns, or changes the
frozen Step 5 result (24 held-out examples, 95.8% full-behavior success,
23 improved / 1 unchanged failure / 0 regressions).

The reusable loading/inference logic lives in `inference/adapted_model.py`:

```python
from inference.adapted_model import AdaptedModel

model = AdaptedModel()               # loads base model + adapter once
result = model.ask(context, question)  # call as many times as needed
```

`AdaptedModel.__init__` loads the tokenizer, the frozen base model
(`local_files_only=True` — no download possible), and attaches the Stage
4 LoRA adapter, then asserts `active_adapters == ["default"]`.
`AdaptedModel.ask(context, question)` builds the exact same prompt shape
the adapter was trained on (reusing `RESPONSE_FORMAT_INSTRUCTION` from
`data_preparation/prepare_dataset.py`, not a re-typed copy) and runs
greedy, deterministic decoding — identical decoding settings to Stages 3
and 5.
...
# Interactive (model loaded once, repeated turns)
python3 inference/run_inference.py
# Then repeatedly:
#   Context: <your context>
#   Question: <your question>
# Blank Context (or Ctrl-D) to quit.

# Fixed demonstration examples (fresh, not from training/test data)
python3 inference/run_inference.py --demo
```

**Demonstrations (fresh examples, not from Stage 2 training or test data —
illustrative only, not part of the frozen Step 5 score):**

```
[Answerable, mining-related, direct extraction]
Context: Antimony production at the mine is 300 tonnes per month.
Question: What is monthly antimony production?
Output:
ANSWER: 300 tonnes per month
CONFIDENCE: HIGH

[Insufficient context, mining-related]
...
[Differently phrased (polar-shaped) question requiring extraction, not yes/no]
...
All three check out: format is correct in every case; confidence behaves
sensibly (HIGH when the fact is stated, LOW when it isn't); the answerable
example extracts the exact supplied figure rather than inventing one; and
the insufficient-context example abstains rather than guessing at a
production figure that was never given. The third example uses the same
polar ("Do you ... ?") question shape that caused Step 5/6's one recorded
failure ("Do you know who authored...?" → "yes") — here it correctly
extracted the entity instead. That's a useful, honest data point, not a
contradiction: it shows the failure mode isn't a deterministic rule but
depends on the specific example, consistent with Step 6's read that
answer-extraction generalized less robustly than format/confidence.

**Plain-English walkthrough — what happens end to end:**

1. **User input:** you type (or pass via `--context`/`--question`) a
   context and a question.
2. **Prompt construction:** `build_prompt()` wraps them in the exact same
   shape as Stage 2's training data — `Context: ...\nQuestion: ...\nInstruction: ...`
   — with no worked examples (zero-shot, like Stages 3 and 5).
3. **Tokenization:** the tokenizer's chat template wraps that prompt in
   Qwen's `<|im_start|>system/user/assistant` turn structure and converts
   the resulting text into token IDs — this is the only place text
   becomes numbers the model can process.
4. **What the frozen base model contributes:** all of the model's
   pretrained language understanding and world knowledge — vocabulary,
   grammar, the general ability to read a passage and answer a question.
   These ~1.546B parameters are unchanged from the original Qwen release.
5. **What the LoRA adapter contributes:** a small set of low-rank updates
   (2.18M parameters, 0.14% of the model) layered onto the attention
   projections. These don't add knowledge — they bias *how* the frozen
   model uses its existing knowledge: defer to the supplied context,
   abstain when it's silent, and respond in the fixed two-line format.
6. **Generation:** `model.generate(...)` runs greedy (`do_sample=False`)
   autoregressive decoding — at each step it picks the single most likely
   next token given everything so far, until it emits an end token or
   hits the token budget. The new tokens are decoded back into text.
7. **Why the adapter doesn't need to be merged into the base model:**
   `PeftModel` inserts the LoRA matrices as extra small modules alongside
   the frozen originals and adds their contribution at forward-pass time;
   the base weights are never edited in place. This keeps the ~2.9 GB base
   model and the ~8.75 MB adapter as two separate, independently
   inspectable artifacts — you can swap or drop the adapter without ever
   touching the base model file, and the base model can be verified
   unchanged (as Stages 4/5 do) at any time.
8. **Why interactive mode reuses the loaded model:** loading the base
   model from disk and attaching the adapter takes several seconds and a
   few GB of memory; doing that once per session and calling `.ask(...)`
